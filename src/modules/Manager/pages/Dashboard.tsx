...
const Dashboard: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    const mostUsedService = services.reduce((prev, current) => {
      return Number(prev.quantities) > Number(current.quantities)
        ? prev
        : current;
    });
  ...
  const exportToPDF = () => {
    ...
    const totalRevenue = services.reduce(
      (sum, service) => sum + Number(service.revenue),
      0
    );
    const totalServiceQuantity = services.reduce(
      (sum, service) => sum + Number(service.quantities),
      0
    );
    ...
    const totalEmployeeRevenue = employees.reduce(
      (sum, emp) => sum + Number(emp.revenue),
      0
    );

    const employeeSummaryData = [
      ["", "Tổng cộng:", "", "", "", "", totalEmployeeRevenue.toLocaleString()],
      [
        bestEmployee.id,
    ...
    const totalProductRevenue = products.reduce(
      (sum, pro) => sum + Number(pro.revenue),
      0
    );
    const totalProductQuantity = products.reduce(
      (sum, pro) => sum + Number(pro.quantities),
      0
    );
