...
import { ACCOUNT, API_URL, APPOINTMENT, BED, BRANCH, CATEGORY, CUSTOMER, EMPLOYEE, SERVICE, SERVICE_CATEGORY, TIME, WORKING_TIME } from "../utils/constants";
...
export const getBedByServiceIdAndDate = async ( serviceId: number | null, date: string | null, branchId: number | null, roomId: number | null) => {
...
export const getCategoryServiceById= async ( categoryServiceId: number | null) => {
  const response = await axios.get(`${API_URL}/${SERVICE}-${CATEGORY}/${categoryServiceId}`
  );
  return response.data;
};
...
export const getAppointmentByCustomerId = async (token: string | null, id: number) => {
  const response = await axios.get(`${API_URL}/${APPOINTMENT}/${CUSTOMER}/${id}`,{
    headers: {
      Authorization: `Bearer ${token}`,
    },}
  );
